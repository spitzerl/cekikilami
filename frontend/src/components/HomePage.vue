  ...
  <div class="relative min-h-screen flex items-center justify-center p-4 overflow-hidden">
    ...
    <div class="relative z-10 w-full max-w-4xl">
      ...
      <div class="text-center mb-8 animate-float">
        ...
        <p class="text-base text-slate-400 font-medium max-w-md mx-auto">
          Le jeu de blind test social interactif. Devinez qui a proposé chaque morceau et devenez le maître du rythme.
        </p>
      </div>

      <!-- Segmented Tab Switcher -->
      <div class="flex justify-center mb-8 opacity-0 animate-fade-in-up" style="animation-delay: 0.05s;">
        <div class="bg-slate-950/60 p-1.5 rounded-2xl border border-slate-800/80 flex gap-1.5 shadow-inner">
          <button 
            ...
            :class="['px-6 py-2.5 rounded-xl text-sm font-extrabold transition-all duration-300 flex items-center gap-2 active:scale-95', activeAction === 'create' ? 'bg-cyan-500 text-slate-950 shadow-lg shadow-cyan-500/10' : 'text-slate-400 hover:text-slate-200']"
          >
          ...
          <button 
            ...
            :class="['px-6 py-2.5 rounded-xl text-sm font-extrabold transition-all duration-300 flex items-center gap-2 active:scale-95', activeAction === 'join' ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/10' : 'text-slate-400 hover:text-slate-200']"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
      ...
      <div class="max-w-md mx-auto opacity-0 animate-fade-in-up" style="animation-delay: 0.15s;">
      ...
      <p v-if="error" class="mt-6 text-center text-rose-400 text-sm font-semibold bg-rose-500/10 border border-rose-500/20 py-2 rounded-lg max-w-md mx-auto">
        {{ error }}
      </p>
    </div>
  </div>
